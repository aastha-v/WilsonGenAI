...
COMMAND = '''bedtools intersect -a mydata_wilsons.bed -b ../pfam_all.bed -wa | awk -F"\t" 'OFS="\t" {print $4, "1"}' - | sort -u - | awk 'BEGIN{print "ID\tPfam_imp_domain"}1' - > op_pfam'''
subprocess.call(COMMAND, shell=True)
...
#shuffle columns based on file that trained our model (header saved in colnames_brca2.csv)
